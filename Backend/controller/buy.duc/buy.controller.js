...
const purchaseItem = async (req, res) => {
  try {
    ...
    try {
      ...
      if (buyer) {
        const fullName = `${buyer.firstName || ""} ${
          buyer.lastName || ""
        }`.trim();
        if (fullName) {
          buyerName = fullName;
      ...
      console.log(
        `Activity logged: Item "${item.name}" purchased by ${buyerName}.`
      );
    } catch (logError) {
...
const confirmBuyItemReceipt = async (req, res) => {
  try {
    ...
    const buy = await Buy.findById(buyId).populate("itemId");
...
const getBuyRecordByItemId = async (req, res) => {
  try {
    ...
    const buy = await Buy.findOne({ itemId }).populate("itemId");
