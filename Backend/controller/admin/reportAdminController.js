...
async function getClerkUsersInfo(clerkUserIds) {
  const uniqueUserIds = [...new Set(clerkUserIds)].filter(id => id && !id.startsWith('IP_'));
  const userMap = {};
    ...
    users.forEach(user => {
      if (user) {
        userMap[user.id] = {
          ...
          email: user.emailAddresses && user.emailAddresses.length > 0 ? user.emailAddresses[0].emailAddress : 'N/A',
...
async function getFormattedTransactions(dateFilter, clerkUserMap) {
  const rawBuys = await Buy.find(dateFilter).populate('itemId', 'owner name images').lean();
  const rawBorrows = await Borrow.find(dateFilter).populate('itemId', 'owner name images').lean();
  ...
  rawBuys.forEach(buy => {
    formattedTransactions.push({
      ...
      item: buy.itemId ? {
        ...
        owner: clerkUserMap[buy.owner] || { id: buy.owner, name: `Unknown (${buy.owner})`, email: 'N/A', isBanned: false },
      } : null,
      user: clerkUserMap[buy.buyer] || { id: buy.buyer, name: `Unknown (${buy.buyer})`, email: 'N/A', isBanned: false },
  ...
  rawBorrows.forEach(borrow => {
    formattedTransactions.push({
      ...
      item: borrow.itemId ? {
        ...
        owner: clerkUserMap[borrow.owner] || { id: borrow.owner, name: `Unknown (${borrow.owner})`, email: 'N/A', isBanned: false },
      } : null,
      user: clerkUserMap[borrow.borrowers] || { id: borrow.borrowers, name: `Unknown (${borrow.borrowers})`, email: 'N/A', isBanned: false },
...
module.exports.getAdminReport = async (req, res) => {
  try {
    const { startDate, endDate, searchUser, searchItem, page = 1, limit = 10 } = req.query;
    ...
    const rawBuysForUserIds = await Buy.find(dateFilter).select('buyer owner').lean();
    rawBuysForUserIds.forEach(b => {
      if (b.buyer) allClerkUserIds.add(b.buyer);
      if (b.owner) allClerkUserIds.add(b.owner);
    });

    const rawBorrowsForUserIds = await Borrow.find(dateFilter).select('borrowers owner').lean();
    rawBorrowsForUserIds.forEach(b => {
      if (b.borrowers) allClerkUserIds.add(b.borrowers);
      if (b.owner) allClerkUserIds.add(b.owner);
    });

    const rawReportsForUserIds = await Report.find(dateFilter).select('userId reportedUserId').lean();
    ...
    let allTransactions = await getFormattedTransactions(dateFilter, clerkUserMap);
    ...
    if (searchItem) {
      ...
      filteredTransactions = filteredTransactions.filter(t =>
        t.item && t.item.name && t.item.name.toLowerCase().includes(searchLower)
      );
    }
    ...
    const topTransactingUsers = Object.entries(transactionCounts)
      ...
      .slice(0, 5)
      .map(([userId, count]) => ({
        user: clerkUserMap[userId] || { id: userId, name: `Unknown (${userId})`, email: 'N/A', isBanned: false },
    ...
    const userBehaviorReports = await Report.find({
      ...dateFilter,
      reportType: { $in: ['user_behavior', 'spam', 'system_generated_violation'] },
      reportedUserId: { $exists: true, $ne: null }
    }).lean();

    userBehaviorReports.forEach(report => {
      const reportedId = report.reportedUserId;
      reportedUserCounts[reportedId] = (reportedUserCounts[reportedId] || 0) + 1;
    });
    ...
    const mostReportedUsers = Object.entries(reportedUserCounts)
      ...
      .map(([userId, count]) => ({
        user: clerkUserMap[userId] || {
          id: userId,
          name: userId.startsWith('IP_') ? `Unknown (IP: ${userId.replace('IP_', '')})` : `Unknown (${userId})`,
          email: 'N/A',
          isBanned: false
        },
        reportCount: count,
    ...
    }).populate('itemId', 'owner').select('itemId rating').lean();
    ...
    const reliableSellers = Object.entries(sellerPerformance)
      .map(([ownerId, stats]) => ({
        user: clerkUserMap[ownerId] || { id: ownerId, name: `Unknown (${ownerId})`, email: 'N/A', isBanned: false },
        ...stats,
        totalCancellations: stats.canceledBuys + stats.canceledBorrows,
        averageRating: stats.ratedReportsCount > 0 ? (stats.summedRatings / stats.ratedReportsCount) : null,
      }))
      .filter(seller => {
        // TIÊU CHÍ MỚI: Ít nhất 5 đơn hoàn thành và không hủy quá 2 đơn
        const meetsTransactionCount = seller.totalTransactions >= 5;
        const meetsCancellationCriteria = seller.totalCancellations <= 2;
        return meetsTransactionCount && meetsCancellationCriteria;
      })
      .sort((a, b) => {
        // THỨ TỰ SẮP XẾP MỚI: Ưu tiên totalTransactions (tổng số đơn đã bán/cho mượn) giảm dần
    ...
    }).select('borrowers status actualReturnTime endTime').lean();
    ...
    const mostProblematicBorrowers = Object.entries(problematicBorrowers)
      ...
      .sort(([, statsA], [, statsB]) => statsB.totalViolations - statsA.totalViolations)
      .slice(0, 5)
      .map(([userId, stats]) => ({
        user: clerkUserMap[userId] || {
          id: userId,
    ...
    const now = new Date();
    const coolDownDate = new Date(now.getTime() - AUTO_REPORT_COOL_DOWN_DAYS * 24 * 60 * 60 * 1000);
    ...
    for (const borrower of mostProblematicBorrowers) {
      if (borrower.totalViolations >= MIN_PROBLEMATIC_BORROW_VIOLATIONS) {
        ...
        if (!existingAutoReport) {
          await Report.create({
            title: `System: Problematic Borrow Behavior Detected for ${borrower.user.name || borrower.user.id}`,
            userId: null,
            reportedUserId: borrower.user.id,
            reportType: 'system_generated_violation',
          ...
          console.log(`Auto-reported problematic borrower: ${borrower.user.name} (${borrower.user.id})`);
        }
      }
    }
    ...
    for (const reportedUser of mostReportedUsers) {
      if (reportedUser.reportCount >= MIN_REPORT_COUNT_FOR_AUTO_FLAG) {
        ...
        if (!existingAutoReport) {
          await Report.create({
            title: `System: User Frequently Reported - ${reportedUser.user.name || reportedUser.user.id}`,
            userId: null,
            reportedUserId: reportedUser.user.id,
            reportType: 'system_generated_violation',
          ...
          console.log(`Auto-reported frequently reported user: ${reportedUser.user.name} (${reportedUser.user.id})`);
        }
      }
    }
  ...
  try {
    ...
    if (report.reportedUserId && !report.reportedUserId.startsWith('IP_')) {
      try {
        ...
        console.warn(`Could not fetch Clerk user ${report.reportedUserId}:`, error.message);
      }
    }
    ...
    if (report.userId) {
      try {
        ...
        console.warn(`Could not fetch Clerk user ${report.userId}:`, error.message);
      }
    }
    ...
    const formattedReport = {
      ...
      reportedUser: reportedUser || {
        ...
        name: report.reportedUserId.startsWith('IP_') ? `Unknown (IP: ${report.reportedUserId.replace('IP_', '')})` : `Unknown (${report.reportedUserId})`,
        email: 'N/A',
        isBanned: false
      },
      reporter: reporter || { id: report.userId || 'N/A', name: report.userId ? `Unknown (${report.userId})` : 'System', email: 'N/A', isBanned: false },
